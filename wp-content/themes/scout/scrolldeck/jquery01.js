...
(function(jQuery) {
    jQuery.scrollorama = function(options) {
		...
		scrollorama.settings = jQuery.extend({}, defaults, options);
		...
		function init() {
			...
			if (typeof scrollorama.settings.blocks === 'string') { scrollorama.settings.blocks = jQuery(scrollorama.settings.blocks); }
			
			// set browser prefix
			if (jQuery.browser.mozilla) { browserPrefix = '-moz-'; }
			if (jQuery.browser.webkit) { browserPrefix = '-webkit-'; }
			if (jQuery.browser.opera) { browserPrefix = '-o-'; }
			if (jQuery.browser.msie) { browserPrefix = '-ms-'; }
			...
			jQuery('body').prepend('<div id="scroll-wrap"></div>');
				...
				if (blocks[i].animations.length) {
					for (j=0; j<blocks[i].animations.length; j++) {
						...
						if (i === currBlockIndex || (currBlockIndex === i-1 && anim.baseline === 'bottom')) {
							...
							if (anim.baseline === 'bottom') { startAnimPos -= jQuery(window).height(); }
							...
							// otherwise, set value based on scroll
								...
								if ( anim.easing && jQuery.isFunction( jQuery.easing[anim.easing] ) ) {
									animPercent = jQuery.easing[anim.easing]( animPercent, animPercent*1000, 0, 1, 1000 );
								}
		...
		scrollorama.animate = function(target) {
			...
			*/
			...
			if (typeof target === 'string') { target = jQuery(target); }
