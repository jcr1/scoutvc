...
(function(jQuery) {
    jQuery.scrolldeck = function(options) {
		...
		var init = function() {
			...
			scrolldeck.settings = jQuery.extend({}, defaults, options);
			
			buttons = jQuery(scrolldeck.settings.buttons);
			slides = jQuery(scrolldeck.settings.slides);
			scrolldeck.controller = jQuery.scrollorama({blocks:slides, offset:scrolldeck.settings.offset});
			
			// add animations with scrollorama
			var anim;
			// ANIMATE INS
			for (i=0; i<jQuery('.animate-in').length; i++) {
				anim = jQuery('.animate-in').eq(i);
			...
			// ANIMATE BUILDS
			for (i=0; i<jQuery('.animate-build').length; i++) {
				anim = jQuery('.animate-build').eq(i);
				switch (anim.attr('data-animation')) {
					case 'fly-in-left':
			...
			for (i=0; i<buttons.length;i++)
				sections.push(slides.index(jQuery(jQuery(buttons[i]).attr('href'))));
			...
			if (lastSlide.outerHeight() < jQuery(window).height()) {
				lastSlide.height(lastSlide.height()+jQuery(window).height()-lastSlide.outerHeight());
			}
		...
		function scrollToSlide(slide) {
			//jQuery(window)._scrollable().stop();
			jQuery(window).scrollTo(slide, {
